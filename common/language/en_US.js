export default {
	'common':{
		name:'Name', idcard:'ID card', gender:'Gender', nationality:'Race', zodiac:'生肖',
		birth:'Birthday',birthTime:'Time of Birth',birthPlace:'Birthplace',isPassaway:'IsPassaway',passingAway:'Date Of death',
		placeResidence:'Address',fixedTelephone:'Home Phone',mobile:'Mobile Phone',emailAddress:'Email',career:'Occupation',
		corporeity:'Physique', bloodType:'Blood Type',gene:'Gene',temperament:'Temperament',constellation:'Zodiac Signs',brief:'My Life',
		years:'years',age:'Age',height:'Height',weight:'Weight',faceShape:'Face Shape',characteristics:'Individual characteristics',
		tshirtSize:'T-shirt Size',shirtSize:'Shirt Size', clothSize:'Cloth Size',trousersSize:'Trousers Size',shoeSize:'Shoe Size'
	},
	'button':{
		editTag:'Add a Label',
		remove:'Remove'
	},
	'login': {
		ctrlName: ['Login', 'Gender', 'Quick login'],
		ohter: ['Verification code', 'Reacquire']
	},
	'index': {
		centerInfo: ['Gender', 'Birthday', 'Race', '生肖']
	},
	'personInfo': {
		title:'Personal Info'
	},
	'appearance':{
		title:'Physical Features'
	}
}
