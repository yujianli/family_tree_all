<template>
	<view>
		...
		<view style="position:relative" v-if="ctrlEnable.tabCtrl"><myTab :tabList="moduleList" @tabSelect="tabSelect" :tabActiveIdx="tabActiveIdx" /></view>
	...
	methods: {
			...
			this.$http.get('content/queryLike', searchParam).then(res => {
				if (res.data.code === 200) {
					...
					for (var i = 0; i < contents.length; i++) {
						...
						if (contents[i].imageUrl) {
							contents[i].imageUrl = this.$common.picPrefix() + contents[i].imageUrl;
						}
					}
		...
		loadMore: function() {
			...
			if (this.isSearch) {
				...
				this.loadContent(this.seledFlagId, true);
			}
		},
		tabSelect(idx) {
